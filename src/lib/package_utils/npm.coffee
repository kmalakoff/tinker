...
module.exports = class Utils extends (require './index')
  @loadModules: (pkg, callback) ->
    Module.destroy {package_id: pkg.id}, (err) ->
      ...
      collectModules = (data) =>
        results = []
        results.push new Module({name: (data.package?.name or ''), path: data.path, root: Utils.moduleDirectory(pkg), git_url: data.package._resolved or data.package.repository?.url})
